...
```bash
npm install -g xno-skills@3.3.4 && npx -y skills add CasualSecurityInc/xno-skills -g -y --all
```
...
```json
{
  "mcpServers": {
    "nano": {
      "command": "npx",
      "args": ["-y", "-p", "xno-skills@3.3.4", "xno-mcp"]
    }
  }
}
```
...
codex mcp add nano \
  ...
  -- npx -y -p xno-skills@3.3.4 xno-mcp
```
</details>
...
```jsonc
{
  ...
  "mcp": {
    "nano": {
      "type": "local",
      "command": ["npx", "-y", "-p", "xno-skills@3.3.4", "xno-mcp"],
      "enabled": true
    }
  }
}
