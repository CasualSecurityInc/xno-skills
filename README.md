...
```bash
npm install -g xno-skills@4.6.1 && npx -y skills add CasualSecurityInc/xno-skills -g -y --all
```
...
```json
{
  "mcpServers": {
    "nano": {
      "command": "npx",
      "args": ["-y", "-p", "xno-skills@4.6.1", "xno-mcp"]
    }
  }
}
```
...
```bash
# Preferred (after npm install -g xno-skills@4.6.1):
codex mcp add nano \
...
codex mcp add nano \
  -c sandbox_mode="danger-full-access" \
  -c 'sandbox_permissions=["network-access"]' \
  -- npx -y -p xno-skills@4.6.1 xno-mcp
```
</details>
