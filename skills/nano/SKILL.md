...
**Preferred — global install** (avoids `npx` concurrency issues that cause handshake failures):
    ...
    npm install -g xno-skills@4.6.1
    xno-skills mcp

**Fallback** (only if global install is not possible):

    npx -y xno-skills@4.6.1 mcp
...
2. bunx -y xno-skills@4.6.1 <command>
3. pnpm dlx xno-skills@4.6.1 <command>
4. npx -y xno-skills@4.6.1 <command>
```
...
```bash
xno-skills --help              # or: bunx -y xno-skills@4.6.1 --help
```
...
- **Supply chain**: Only use `xno-skills@4.6.1` and `@open-wallet-standard/core`. No other npm packages.
...
```bash
bunx -y xno-skills@4.6.1 balance --wallet "my-wallet"
bunx -y xno-skills@4.6.1 rpc account-balance <address>
```
...
**Via CLI (required flags only):**
```bash
bunx -y xno-skills@4.6.1 receive --wallet "my-wallet"
```
...
**Via CLI (required flags only):**
```bash
bunx -y xno-skills@4.6.1 send --wallet "my-wallet" --to "nano_..." --amount-xno 0.01
```
...
```bash
bunx -y xno-skills@4.6.1 qr nano_1abc...
```
...
**Via CLI:**
```bash
bunx -y xno-skills@4.6.1 validate nano_1abc...
```
...
**Via CLI:**
```bash
bunx -y xno-skills@4.6.1 convert 1 xno       # all units
bunx -y xno-skills@4.6.1 convert 1 knano
bunx -y xno-skills@4.6.1 convert 1000000000000000000000000000000 raw
bunx -y xno-skills@4.6.1 convert 1 xno --json
```
...
bunx -y xno-skills@4.6.1 sign "<message>" --key YOUR_PRIVATE_KEY_HEX

# Sign with JSON output
bunx -y xno-skills@4.6.1 sign "<message>" --key YOUR_PRIVATE_KEY_HEX --json
```
...
```bash
# Verify
bunx -y xno-skills@4.6.1 verify <nano_address> "<message>" <signature-hex>

# Verify with JSON output
bunx -y xno-skills@4.6.1 verify <nano_address> "<message>" <signature-hex> --json
```
...
```
```bash
bunx -y xno-skills@4.6.1 change-rep --wallet "my-wallet" --representative "nano_..."
```
